...
- **Fix**: _(pending)_ Correct the SRI hash, remove the `integrity` attribute for this stylesheet, or vendor the Leaflet CSS locally and reference that copy.
...
- **Fix**: _(pending)_ Add a small favicon asset and `<link rel="icon" href="...">`, or explicitly decide to ignore it during prototype testing.
...
- **Fix**: _(pending)_ Add clustering, larger hit targets at lower zooms, a synchronized records table, or a "select nearest crash" interaction that queries nearby records.

...
- **A1 — HIN "one sentence" hero (small).** "X% of DC streets account for Y% of people killed or seriously injured," from the DC HIN + crash KSI counts, source-linked. _Data: HIN layer + crashes joined to HIN segments._
- **A2 — Headline scorecard with baseline + accountability (small).** Deaths/serious injuries YTD, each with % change vs. a stated baseline (2015 launch or 2017-19 average), plus the honest "missed 2024 target" note. _Data: crash fatal/major flags by year._
- **A3 — Multi-year trend with the "fewer injuries, more deaths" story (small).** Fatalities vs. injuries over time, annotated with the COVID-era divergence already documented in CLAUDE.md. Motivates the KSI focus.

...
- **`hotspots.html` — High-Injury Corridors.** Leaflet map + profile cards for the highest-injury corridors (`data/hotspots.geojson`), plus an all-77-corridor sortable table from `data/hin-corridors.json`: severity, mode breakdown, recommended interventions, equity notes, confidence, and scan-friendly KSI/death/crash columns. Per-corridor counts are real — `pipeline/hotspots.py` joins crash records to the DDOT High Injury Network (crashes within 25 m of the centerline, 2022-present).
...
- **`laws.html` — Safe Streets Law Tracker.** Source-linked inventory of enacted DC street-safety laws and codified funds, with delivery pointers for tracking what was promised versus what is visible in the data. Also carries a ten-year law-lineage panel, a "Pending & Proposed" bills surface (`data/bills.json`, LIMS-sourced and kept separate from enacted law — statuses badge-gated as unverified until LIMS-confirmed), and a "Who can act — and when to show up" coalition / pitch-target surface (`data/pitch-targets.json`) that maps each finding type to the body that can act on it, the channel, and the DC calendar moment, with the bike-lane-opposition landscape flagged.
...
Curated (hand-authored, source-linked) files back the recommendation and law views: [data/countermeasures.json](data/countermeasures.json) (proven fixes with cited effect sizes; figures are flagged unverified until checked against the primary source), [data/recommendations.json](data/recommendations.json) (evidence cards following the CLAUDE.md recommendation fields), [data/legislation.json](data/legislation.json) (verified enacted legal authority with delivery-audit pointers), [data/bills.json](data/bills.json) (pending & proposed legislation, LIMS-sourced and kept separate from enacted law — every status flagged `status_verified:false` until LIMS-confirmed — plus the ten-year enacted-law lineage), and [data/pitch-targets.json](data/pitch-targets.json) (coalition & pitch-target inventory, web-verified 2026-07-09, mapping each finding type to who can act, the channel, and the DC calendar moment).
...
`node --test` also runs the data-integrity suites for the law tracker and its policy surfaces: [tests/legislation-data.test.mjs](tests/legislation-data.test.mjs), [tests/bills-data.test.mjs](tests/bills-data.test.mjs) (enforces that pending bills never link the enacted-law domain and that unverified statuses are badge-gated), and [tests/pitch-targets-data.test.mjs](tests/pitch-targets-data.test.mjs) (source URLs present, references resolve, opposition landscape flagged). Planning and decision docs live alongside the code: [PLAN.md](PLAN.md), [DEPLOYMENT.md](DEPLOYMENT.md), and [PERF.md](PERF.md).
